...
string stringFromAny(const any& v) {
	if (v.type() == typeid(char const*))
		return string(any_cast<char const*>(v));
...
any anyFromField(const gpb::Reflection* refl, const gpb::FieldDescriptor* field,
...
void anyToField(gpb::FieldDescriptor::CppType cpp_type,
                const gpb::Reflection* refl, const gpb::FieldDescriptor* field,
                const any& a, gpb::Message& msg) {
		...
		if (auto oneof_desc = field->containing_oneof()) {
			if (oneof_set.count((void*)oneof_desc) == 0) {
				if (get_options) {
					...
					oneof_set.insert(
					    (void*)oneof_desc);  // save oneof handle so repeated
			...
			if (!field->is_repeated()) {
				auto& submsg = refl->GetMessage(
				    m, field);  // for nested messages, option case and value
				                // case are to just recurse
				ret.emplace(field->name(),
				            protobuf2anymap(submsg, get_options));
			}
			...
			} else {  // non-option case returns a pair including the int and
				      // string names
				const auto& enum_value = refl->GetEnum(m, field);
				ret.emplace(field->name(), make_pair(int(enum_value->number()),
				                                     enum_value->name()));
			}
		} else {
			if (get_options) {  // if its a scalar field, option is just the C++
				                // type name
	...
	bool process(const AnyMap& am, gpb::Message& msg) {
		...
		for (const auto& [amk, amv] : am) {
			...
			if (field->cpp_type() == gpb::FieldDescriptor::CPPTYPE_ENUM) {
				if (amv.type() == typeid(string) ||
				    amv.type() == typeid(char const*)) {
					auto v = stringFromAny(amv);
					auto ptr = field->enum_type()->FindValueByName(v);
					if (ptr == nullptr) {
						stringstream ss;
						ss << "Cannot lookup enum name=" << v
					...
					refl->SetEnumValue(&msg, field, any_cast<int>(amv));
			           ...
			           gpb::FieldDescriptor::CPPTYPE_MESSAGE) {
...
void anymapFormat(ostream& os, const AnyMap& m, const string& pad, size_t depth,
                  bool with_type) {
	if (depth == 0)
		os << padX(pad, depth) << "{\n";
	for (const auto& [k, v] : m) {
		if (v.type() == typeid(AnyMap)) {
			os << padX(pad, depth + 1) << k << " = "
			   << "{\n";
			anymapFormat(os, any_cast<AnyMap>(v), pad, depth + 2, with_type);
			os << padX(pad, depth + 1) << "}\n";
		} else {
