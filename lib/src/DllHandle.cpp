...
struct DllHandle::Impl {
	...
	Impl(const std::string& path, const WhiteList& white_list = WhiteList())
		...
		// struct ::link_map *lmp;
		// if (dlinfo(dl_handle, RTLD_DI_LINKMAP, &lmp) != 0)
		// throw_error("dlinfo(RTLD_DI_LINKMAP)", path); for ( ; lmp; lmp =
		// lmp->l_next) {
		//     using namespace std;
		//     cout << "MD5: " << compute_md5(lmp->l_name)  << ' ' <<
		//     lmp->l_name  << endl;
		// }
	}

	~Impl() {
		// apparently, dlclose with actually not unload or even run dtors if
		// there are 'UNIQUE' tagged objects So, don't depend on dtors in plugin
		// code for back-to-back clean testing.
		if (dl_handle != nullptr) {
			if (dlclose(dl_handle) != 0)
				throw_error("dlclose", path);
		}
	}
