...
struct DiskWriter::Impl {
	...
	explicit Impl(const string& path) {
		...
		if (index_fd < 0) {
			stringstream ss;
			ss << "DiskWriter failed due to \"" << getErrno(errno)
			   << "\" for \"" << index_path << '"';
	...
	void commit(double timeStamp, uint64_t marker, const json& payload) {
		...
		if (ret < 0) {
			stringstream ss;
			ss << "DiskWriter::commit failed due to \"" << getErrno(errno)
			   << "\" for \"" << index_path << '"';
			throw runtime_error(ss.str());
		}
...
void DiskWriter::commit(double timeStamp, uint64_t marker,
...
struct DiskReader::Impl {
	...
	explicit Impl(const string& path) {
		...
		if (index_fd < 0) {
			stringstream ss;
			ss << "DiskReader failed due to \"" << getErrno(errno)
			   << "\" for \"" << index_path << '"';
			close(cbor_fd);
			throw runtime_error(ss.str());
		}
	...
	json read(size_t index, double& timeStamp, int64_t& marker) {
		...
		if (ret <= 0) {
			stringstream ss;
			ss << "DiskReader::read failed due to \"" << getErrno(errno)
			   << "\" for \"" << index_path << '"';
			throw runtime_error(ss.str());
		}
